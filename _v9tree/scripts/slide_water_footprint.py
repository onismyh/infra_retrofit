...
def main() -> None:
    ...
    # 幻灯版：只保留冷却方式水强度（c）与流域径流变化地图（d），上下堆叠
    fig = plt.figure(figsize=(5.4, 6.0))
    grid = fig.add_gridspec(2, 1, height_ratios=[0.62, 1.0], hspace=0.30,
                            left=0.11, right=0.97, top=0.965, bottom=0.03)
    ax_a = fig.add_subplot(grid[0, 0])
    ax_c = fig.add_subplot(grid[1, 0])
    ax_c.set_anchor("N")

    panel_a(ax_a, intensities)
    panel_c(ax_c, sites, drying)

    panel_label(ax_a, "c")
    panel_label_inside(ax_c, "d", x=-0.005, y=1.02)

    save_fig(fig, "water_cd")
