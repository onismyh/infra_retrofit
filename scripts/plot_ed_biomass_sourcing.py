...
def panel_map(ax, f: pd.DataFrame) -> None:
    ...
    ax.set_title(f"{MAP_YEAR} 年全国燃用生物质 {d['flow_gj'].sum() / 1e9:.1f} EJ，其中 {self_pct:.0f}% 产自\n"
                 f"同一流域，流量加权运距 {haul:.0f} km"
...
def panel_self(ax, f: pd.DataFrame) -> pd.DataFrame:
    ...
    ax.set_yticks(y)
    ax.set_yticklabels([f"{b} {BASIN_NAMES_ZH.get(b, EP.BASIN_NAMES.get(b, b))}" for b in g["dst_basin"]],
                       fontsize=5.8)
