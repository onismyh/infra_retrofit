...
def panel_age(ax, fleet: pd.DataFrame) -> None:
    ...
    ax.set_title("机组多数在 2005 年之后投产——\n"
                 "退役会毁掉资产价值，改造不会", fontsize=6.8, linespacing=1.25)
...
def main() -> None:
    ...
    for ax, lab, x, y in ((ax_map, "a", 0.005, 0.985), (ax_cool, "b", -0.135, 1.10),
                          (ax_age, "c", -0.135, 1.12)):
        ax.text(x, y, lab, transform=ax.transAxes, fontsize=8.0, fontweight="bold",
                va="top", ha="left")

    save_fig(fig, "ed_fig1_fleet_atlas", subdir="extended")

    con = fleet[fleet["constrained"].fillna(False)]
    print(f"  sites {len(fleet)}, units {fleet['unit_count'].sum():,.0f}, "
          f"capacity {fleet['capacity_gw'].sum():,.1f} GW")
    print(f"  constrained basins: {con['capacity_gw'].sum():.1f} GW "
          f"({100 * con['capacity_gw'].sum() / fleet['capacity_gw'].sum():.1f}%), "
          f"{len(con)} sites")
    for tech in ("once_through", "recirculating", "air"):
        print(f"  {COOLING_LABEL[tech]:>16}: {fleet[f'capacity_mw_{tech}'].sum() / 1e3:7.1f} GW")
