...
BASES = [
    ("dry_season_water_m3_per_year", "枯水期——约束真正作用的口径"),
    ("available_water_m3_per_year", "年均值——多数文献使用的口径"),
]
...
def draw_basis(ax, ax_delta, table: pd.DataFrame, binding: list[str], title: str,
    ...
    for basin in table.index:
        name = "全国" if basin == "National" else f"{basin} {BASIN_NAMES_ZH.get(basin, basin)}"
