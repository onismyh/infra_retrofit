"""旧求解树（v9.1）的路径引导：`ROOT` 是本树根，`SRC` 是仓库根的 `src/`。
...
另一个模型（工业强制开启、部门碳目标、新成本口径与期末残值）：`run_*` 复现不了 v9.1 的解，
`build_*` 会用新的管网构建器覆盖本树冻结的 `inputs/`。这两类入口在这里直接拒绝；
需要复现时，在删除前的提交上另开一份工作副本（那里树内 `src/` 还在）：

    git worktree add ../infra_v9.1 892c877^
    cd ../infra_v9.1/_v91tree && python scripts/run_single.py <name> --threads 8
"""
...
if _ENTRY.startswith(("run_", "build_")):
    raise SystemExit(
        ...
        "复现请用删除前的提交：git worktree add ../infra_v9.1 892c877^"
    )
