...
EXPERIMENTS: dict[str, tuple[dict, dict]] = {
    ...
    # --- JOINT COAL + INDUSTRY ------------------------------------------------------
    # A DIFFERENT MODEL, not a knob on the one above. `include_industry` adds 390 hubs to
    # the CO2 network, to the basin water cap, and to a SINGLE joint emission target whose
    # denominator becomes coal 5 392 + industry 3 269 = 8 661 Mt. The same target fraction
    # therefore demands 61% more absolute abatement.
    #
    # Hence the `IND_` PREFIX rather than a suffix: in this repo a suffix (_noair,
    # _capfree, _seedN) has always meant one knob on one model, and those runs may be
    # differenced. An `IND_` run may NEVER be differenced against a non-`IND_` run --
    # the objective, the target and the source population all changed at once.
    #
    # The pair below is the minimum closed loop the author asked for on 2026-09-07:
    # no water rule, then the full official-quota basin cap, so that what industry does to
    # the water constraint is visible on its own.
    # TARGET LEVEL, set by the author on 2026-09-08: the 2060 ABSOLUTE abatement is held at
    # what the coal-only calibration demanded, so the joint runs carry the same ambition rather
    # than 1.61x of it. Derivation, reproducible from the two baselines:
    #
    #     0.95 x 5392.1966 (coal) / 8661.4434 (coal + industry) = 0.591424 -> 0.5914
    #     required 2060 abatement 5122.38 Mt vs the coal-only 5122.59 Mt (-0.21 Mt)
    #
    # Keeping 0.95 on the joint denominator would have demanded 8228 Mt against a technical
    # ceiling near 8334 Mt that is further squeezed by 1282 Mt/yr of national injectivity --
    # cement and EAF steel (1121 Mt) have no hydrogen route and must capture. 2060 would then
    # be a forced solution reported through a big-M shortfall, not an optimisation.
    "IND_BASE": ({"mip_gap": 0.01, "emission_target_fraction": (0.0, 0.0, 0.0, 0.5914)}, {"include_industry": True}),
    "IND_WA_cwatm_126_dry_oq": ({"water_mode": "grid_supply", "water_scenario_id": "cwatm|gfdl-esm4|ssp126", "water_season": "dry", "mip_gap": 0.01, "emission_target_fraction": (0.0, 0.0, 0.0, 0.5914)}, {"water_budget": "official_quota", "include_industry": True}),

    # --- BINDING joint target, `_t95` -------------------------------------------------
    # Measured on the 0.5914 pair (solved 2026-09-08): the joint target IS NOT BINDING there.
    # 2060 demands 5122.4 Mt and coal alone delivers 5923.4 Mt at its own economic optimum,
    # because the carbon price already pays for abatement past the target. Industry then has
    # no carbon reason to act, does nothing in IND_BASE, and the "single joint target" has
    # nothing to allocate -- so no industrial CO2 ever entered the pipeline network either.
    #
    # The target only starts to bite above 5923.4 / 8661.4 = 0.684. At 0.95 it demands
    # 8228 Mt, leaving ~2300 Mt for industry -- inside its technical range (2942 Mt at full
    # CCS) but competing with coal for the 1282 Mt/yr of national injectivity. THAT is the
    # regime the joint model exists to study, and the one that exercises source-sink matching.
    #
    # An earlier note here claimed 0.95 was infeasible. That rested on taking coal's ceiling
    # to be its own baseline (5392 Mt); it is not, because BECCS carries a reduction factor of
