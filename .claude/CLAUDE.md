...
## 〇、工作流程（作者要求，2026-09-22）

> **注：与作者交流一律用中文**——计划、进度汇报、差异说明、提问、最终总结，全部用中文写。

### 动手前

1. **先列计划，批准再动手**
   开始任何任务前，先列出执行步骤并等待明确批准。计划未经确认，不得实施。

2. **改之前先读文件**
   编辑任何文件之前，必须先读取该文件的当前内容，不能只靠记忆或上下文。

3. **优先复用，不重复造轮子**
   先寻找项目已有的抽象和函数；确认没有合适实现后，再新建代码。

### 执行中

4. **不确定先说，不要猜**
   没有可参考的先例时，停下来询问。不要自行发明需求或填补缺失信息。

5. **中途转向，先问再动**
   发现原方案走不通或范围发生变化时，先汇报新方案并获得确认，再调整计划。

6. **计划外的问题先报告**
   遇到与当前任务无关的废弃代码、可疑行为或其他问题，单独列出报告，严禁擅自修改。

### 交付前

7. **完整展示改动**
   提交前展示完整的文件变更差异，说明修改了哪些文件和行，并获得明确批准。

8. **未通过验证不算完成**
   宣布就绪前，针对改动的包运行最小范围的检查，包括 lint、类型检查和测试。
...
| 高耗能工业 | 钢铁（长流程 / 电炉）、水泥、合成氨、甲醇，共 390 个 hub。**乙二醇、烯烃、炼化不在模型内**（缺取水定额与路线参数，`SECTORS_OUT_OF_SCOPE`） |

### 1.2 改造路径（decision）

- **掺氨**（co-firing，含掺烧比例决策）。**煤电侧没有掺氢路径**（煤粉锅炉掺氢无工程案例，作者 2026-09-10 决定不加）；工业侧钢铁 / 合成氨 / 甲醇有氢路线
...
2. **MIPGap 分层。** 进主图的头部情景与 seed 复现族用 registry 的 1%；仅做背景的情景可放宽到 3%。
   ...
   **例外：`ST_` 系（部门碳目标）统一 3%**（作者决定 2026-09-11）。该系的整数管径根数带固有整数间隙
   （下界里小流量边用分数根 20 Mtpa 大管，单位成本只有整根 2 Mtpa 小管的 40%），hub 决策连续化后
   两个头部情景 5 min 到 2.7–3.0%，之后 2 h 可行解与下界都不动，1% 不可达。所以 `ST_` 的任何差值
   **只能按第 3 条的可证区间报告**，不得直接相减；求解流程必须走 LP 松弛热启动（实现说明 §9.7）。
...
5. **并发求解最多 2 路**。历史原因是 `_biomass_access_matrices` 的 13,949 × 71,148 稠密矩阵（7.39 GiB）；
...
7. **成本口径（作者决定 2026-09-22）：煤电与工业的每个减排选项都按"改造 capex（一次性，计在存量增量上）
   ...
   平准化数里的资本回收假设与之矛盾。ACCA21 区间只作交叉核对。参数出处在 `docs/工业部门参数溯源.md` §八。
   `_indtree` 结果按落盘时间分三段，段与段之间不得相减：2026-09-22 之前是平准化旧口径；09-22 起到 PR #2 合入
   `feature/industrial-sectors` 之前，是上述口径但还没有 PR #2 在 09-23 做的改动（工业 capex 改计在能力存量增量上、删掉 BECCS 重复计费、管道到寿命可在原址重铺、氨价里的合成岛年金按情景贴现率重算、北京煤价 69.4 → 38.6 元/GJ、长流程钢氢路线减排比例 0.85 → 0.95、合成岛年金寿命 20 → 30 年，
   见 README §0.1、§0.2）；合入之后才是现行口径。结果目录不记提交号，按落盘时间与合并提交的时间判断。
...
| 工业氢路线（H₂-DRI / 绿氢合成） | `#9E9AC8` | Purples 浅端 |
| （预留）掺氢 + CCS，模型中无此路径 | `#756BB1` | Purples 深端 |
| 退役 | `#D8DCE0` | 中性灰 |
| 空冷改造（叠加线） | `#CC3311` | 强调红 |
