...
CCS 路线（`constants_industry.INDUSTRY_CCS_*`）：按每吨年捕集能力计的 capex，水泥 1 150、钢铁 1 000
CNY/(t/a) 取自中国项目备案；高浓度化工 450 为 ⚠ 假设（由全成本反推，按注释里的输入只得约 405）。
固定运维 5%/a；再生蒸汽由厂内燃煤锅炉产生，按 hub 所在省的煤价计价；电按情景电价；耗材 15 或
5 CNY/t。蒸汽的 CO2 直接放空，并从该路线的减排量中扣除（2026-09-22 之前把 ACCA21 单位成本视为
已含能耗，也没有放空任何 CO2）。煤电侧的学习曲线照旧缩放 capex（固定运维随之缩放）。
