...
`optimization/data_prep.py` 记录了后两个因子无法分别识别（aliased）——求解器只看得到它们的
乘积（0.03）——所以本研究中没有任何实验能说清起约束作用的是环境流量标准还是分配规则。更糟
的是，分母是*本地天然径流*，对华北各流域而言这根本就是错误的量：海河流域每年实际消耗的水都
...
刻意不包含的内容。
  * 逐流域上限。国办发 2013 no.2 attachment 1 是一个闭合的省级划分（31 行合计恰为
