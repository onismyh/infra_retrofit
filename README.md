...
> 批 4（PR #2 审查时列出的 20 项表外参数逐项查文献，按作者拍板补出处或标 `⚠ 假设` 并附对照值，见 §0.2 末）。
> 标"未改"的地方要不要统一，由作者决定。
...
结果见本节末的"表外参数"表；查的过程中，下面工业残值寿命与管道 capex 两行的标签也随之改了。
...
| 氢路线减排比例：长流程钢 | 0.85 | 对路线化学的判断 | ⚠ 假设（无出处） |
...
|---|---|---|
| 文件数 | 55 | 58（工业侧拆出 3 个） |
| 中文行 / 英文行 | 449 / 1 610（22%） | 1 916 / 81（96%） |
...
1. **仍超 400 行的 7 个文件**：`builders/water.py` 736、`constants_industry.py` 604、`optimization/scenario.py` 602、
