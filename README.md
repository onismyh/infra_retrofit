...
为 37.2，缺省值没有跟改，改标 ⚠ 设定值。仓库根 hub 表里用到它的是写作 "Neimenggu" 的 28 个 hub（煤价表里是
...
多来自 2026-09-10（煤电）与 09-22（工业）两轮检索，不是每条都核过原文：核过本地 PDF 或开源数据的在出处栏写了表号，没核过的在判定
里注"原文未核"。"推导"= 由有出处的数反推；"⚠ 假设"= 只有取值理由，没有可查的文献或项目（括号里说明是无出处、出处不具体还是设定值）。
...
| 技术 | 参数 | 取值 | 出处 | 判定 |
|---|---|---|---|---|
| CCS | 捕集岛 capex（2030，含压缩） | 3 500 元/kW | 袁家海等 2022（主引）；Lockwood 2018 IEA CCC（上沿）；An et al. 2025 SI Table 7（下沿，2 140 元/kW）；国能锦界 2024 备案。本地 PDF 里另有两个改造 capex，注释原来没提：Fan et al. 2023 SI Table 15 为 4 373 元/kW，Wang et al. 2025 SI Table 3 为 1 559.75（205–2 914.5）$/kW ≈ 10 918（1 435–20 402）元/kW | 有出处（主引与上沿原文未核；本地能核的三个值 2 140 / 4 373 / 10 918） |
...
| | 运维 | 30 元/MWh | Wang et al. 2025 SI Table 3 的固定运维 γ2 = 18.85（5.5–32.2）$/(kW·a) ≈ 132 元/(kW·a)，按约 4 400 h 折成每 MWh；同表可变运维 γ3（27.5 $/kW）未计 | 推导（只计 γ2；4 400 h 无出处，按模型全机组平均 4 643 h 折为 28.4） |
| 掺氨 | 掺烧升级 capex | 2.5 万元/MW/档（满档 50% 掺烧 125 元/kW） | CNERI 2025（90 元/kW，100% 改造另加储存）、Deng et al. 2024（163 元/kW，供应系统）、Li & Li 2022（121.6 元/kW，只算燃烧器，未核实）；三个值口径各异，都不是 50% 掺烧，125 是中位数 121.6 取整 | 水平有出处（原文未核）；逐档线性的形状 ⚠ 假设（无出处） |
| | 运维 | 80 元/MWh | — | ⚠ 假设（无出处） |
...
|---|---|---|
| 文件数 | 55 | 58（工业侧拆出 3 个） |
| 中文行 / 英文行 | 449 / 1 610（22%） | 1 923 / 81（96%） |
